...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('tr[data-playing=true]').count(), 30);
    ...
    assert.equal(await page.locator('tbody tr:visible').count(), 30);
    ...
    assert.equal(await page.locator('details[open]').count(), 30);
    ...
    console.log('Browser audit passed: 158 rows, 30 animations, search/filter, pause/play, all 32 copper frames, stepping, reduced motion, no external requests/errors.');
  } finally {
    await browser.close();
  }
