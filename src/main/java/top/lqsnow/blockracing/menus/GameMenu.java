...
public final class GameMenu extends MenuView {
    public GameMenu() {
        ...
        setButton(12, MenuButton.of(
                player -> ItemBuilder.of(Material.WRITABLE_BOOK)
                        .name(Message.MENU_TARGET_LIST.getString(player))
                        .lore(Message.MENU_TARGET_LIST_LORE.getStringList(player)).build(),
                (player, click) -> new TargetListMenu(player, 0).open(player)
        ));
        ...
        setButton(21, MenuButton.of(
                player -> ItemBuilder.of(Material.ENDER_PEARL)
                        ...
                        .lore(replaceScorePlaceholder(Message.MENU_RANDOM_TP_LORE.getStringList(player)))
                        .build(),
    ...
    private static void sendBlockSection(Player player, List<String> blocks) {
        for (int index = 0; index < blocks.size(); index++) {
            ...
            player.sendMessage(Message.NOTICE_BLOCK_OVERVIEW_ENTRY.getString(player)
                    ...
                    .replace("%block%", Game.getTargetDisplayName(block, player)));
        }
    }
    ...
    private static List<String> replaceScorePlaceholder(List<String> lore) {
        return lore.stream()
                .map(line -> line.replace("%score%", String.valueOf(Setting.getLocateCost()))
