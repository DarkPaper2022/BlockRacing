...
- 队伍共享目标进度已合入 `main`：累计求和、唯一目标去重；保留 `feature/shared-team-progress` 开发分支，详见[共享规则与验证](docs/shared-team-progress.md)。
...
可选 Fabric 客户端模组在独立 `client-mod/` Git 子模块中开发：按 Tab 打开 8×8 本局目标面板，Bonus 独立展示，服务端只读同步。初始化使用 `git submodule update --init --recursive`；详见[客户端目标面板设计与构建](docs/client-task-board.md)。

764 个目标的中文、requirement 与代码判定静态核对见[一致性审计报告](docs/audits/target-consistency-20260906/summary.md)和[逐项结果表](docs/audits/target-consistency-20260906/targets.csv)；后续规则修复与累计进度复查见[修复记录](docs/audits/target-consistency-20260906/fixes.md)。
