...
- `简单`：从 `100` 线性下降，并在 `progress >= 0.75` 时直接停出
- `普通`：前 `40%` 从 `20 -> 60`，后 `60%` 从 `60 -> 80`
...
- 把 `EASY_DISABLE_PROGRESS` 维持在 `0.70f ~ 0.80f`
...
- `progress = 0.00`：`easy=100`，`medium=20`，`hard=1`
- `progress = 0.40`：`easy=46`，`medium=60`，`hard=24`
- `progress = 0.75`：`easy=0`，`medium=71`，`hard=60`
- `progress = 0.90`：`easy=0`，`medium=76`，`hard=78`

这意味着当前服的后期目标不会再出 `简单`，而 `普通` 和 `困难` 会继续抬高。
